...
def extract_data_from_pdf(text, year):
    ...
    table_number_match = re.match(r'^(?:[Tt]able) (\d+[A-Z]?)', lines[0])
    ...
    # Extract table title (ends at the line containing the year in different formats)
    title_lines = []
    year_pattern = re.compile(rf'{year // 100}\s*{year % 100}|{year}')
    for line in lines[1:]:
...
# Extract and save tables from the PDF
try:
    pdf_filename = 'vital_stats_1999.pdf'
            ...
            if text:
                ...
                if re.match(r'^[Tt]able \d+[A-Z]?', text.strip()):
                    ...
                    if table_count[table_number] == 1:
                        excel_filename = os.path.join(output_directory, f"Table_{table_number}.xlsx")
                    else:
                        excel_filename = os.path.join(output_directory, f"Table_{table_number}_{table_count[table_number]}.xlsx")
